...
class DependentFilteredEntitySubscriber implements EventSubscriberInterface
{
    ...
    public function __construct($container, PropertyAccessorInterface $propertyAccessor = null)
    {
    ...
    public function preSetData(FormEvent $event)
    {
        ...
        foreach($this->fields as $name => $options) {
            $object = $data;
            $parent = ($object) ? $accessor->getValue($object, $options['parent_field']): null;
    ...
    private function addDependantForm($form, $parent_id, $options)
    {
        ...
        $form->add($field_name,
            'apptools_dependent_filtered_entity', $options);
    }
}
